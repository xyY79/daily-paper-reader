...
tags: ["query:sr"]
score: 8.0
evidence: 索引阶段用于多跳问答的跨文档推理
tldr: IndexRAG 针对多跳问答中跨文档推理效率低的问题，提出将推理过程从在线推理转移到离线索引阶段。通过识别文档间的桥接实体并生成独立的桥接事实，该方法无需额外训练即可在推理时仅通过单次检索和单次大模型调用实现高效推理。实验证明其在多个基准测试上显著优于 Naive RAG 和主流图检索方法，兼顾了效率与准确性。
selection_source: fresh_fetch
motivation: 现有的多跳问答 RAG 方法通常依赖复杂的在线图处理或多次迭代推理，导致推理效率和计算成本较高。
method: 在索引阶段识别跨文档的桥接实体并预先生成桥接事实作为独立检索单元，从而将跨文档推理逻辑前置到离线阶段。
result: 在三个多跳问答基准测试中，IndexRAG 的 F1 值比 Naive RAG 平均提高 4.6 分，且在结合 IRCoT 时性能优于 HippoRAG 等图基准。
conclusion: IndexRAG 证明了通过在索引阶段预处理跨文档关联，可以在保持单次检索效率的同时显著提升大模型的多跳推理能力。
---

## 摘要
多跳问答（QA）需要跨多个文档进行推理，然而现有的检索增强生成（RAG）方法要么通过需要额外在线处理的基于图的方法，要么通过迭代的多步推理来解决这一问题。我们提出了 IndexRAG，这是一种将跨文档推理从在线推理转移到离线索引的新颖方法。IndexRAG 识别跨文档共享的桥接实体，并将桥接事实生成为可独立检索的单元，且无需额外的训练或微调。在三个广泛使用的多跳问答基准测试（HotpotQA、2WikiMultiHopQA、MuSiQue）上的实验表明，IndexRAG 的 F1 分数比 Naive RAG 平均提高了 4.6 个百分点，同时在推理时仅需单次检索和单次大语言模型（LLM）调用。当与 IRCoT 结合时，IndexRAG 在平均表现上优于所有基于图的基准模型，包括 HippoRAG 和 FastGraphRAG，而它仅依赖于扁平检索。我们的代码将在论文被接收后发布。
