...
tags: ["query:sr"]
score: 9.0
evidence: 用于精确证据检索的深度研究框架
tldr: 针对深度搜索代理中推理查询与网页索引结构不匹配的问题，本文提出WeDas框架。该框架引入“查询-结果对齐分数”来衡量意图与检索结果的兼容性，并利用少样本探测机制动态估计该分数，使代理能根据网页内容分布实时调整搜索策略。作为即插即用模块，WeDas在多个基准测试中显著提升了搜索准确率和任务完成度，有效弥合了高层推理与底层检索间的差距。
selection_source: fresh_fetch
motivation: 现有深度搜索代理的推理查询与网页索引结构不匹配，导致检索结果过于粗略或过于细碎，难以获取精确证据。
method: 提出WeDas框架，利用查询-结果对齐分数和少样本探测机制，使代理能够感知网页内容分布并动态校准子目标。
result: 在四个基准测试中，WeDas作为即插即用模块一致性地提高了子目标的完成度和检索准确性。
conclusion: 通过将搜索空间结构特征纳入代理的观察空间，该研究成功弥合了高层推理与底层检索之间的鸿沟。
---

## 摘要
尽管集成了搜索工具，深度搜索智能体（Deep Search Agents）往往面临推理驱动的查询与底层网页索引结构之间不匹配的问题。现有框架将搜索引擎视为静态工具，导致生成的查询要么过于粗略，要么过于细碎，难以检索到精确的证据。我们提出了 WeDas，一个感知网页内容分布的框架，它将搜索空间的结构特征纳入智能体的观察空间。我们方法的核心是查询-结果对齐分数（Query-Result Alignment Score），这是一种量化智能体意图与检索结果之间兼容性的指标。为了克服对动态网页进行索引的困难，我们引入了一种少样本探测机制，通过有限的查询访问迭代估计该分数，使智能体能够根据局部内容格局动态重新校准子目标。作为一种即插即用的模块，WeDas 在四个基准测试中持续提高了子目标的完成度和准确性，有效地弥合了高层推理与底层检索之间的鸿沟。
