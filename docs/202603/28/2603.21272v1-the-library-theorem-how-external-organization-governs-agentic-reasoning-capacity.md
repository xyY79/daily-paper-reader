...
tldr: 本研究探讨了结构化外部索引对大模型智能体推理的影响。通过将上下文窗口形式化为I/O页面，证明了索引化存储能将检索成本从线性降低至对数级。实验显示索引化智能体效率极高，但也揭示了“参数化记忆竞争”现象，即模型因依赖内部知识而跳过检索协议。研究建议将LLM用于索引构建，而将索引遍历交给确定性算法，以提升推理的可靠性与效率。
selection_source: fresh_fetch
motivation: 探讨如何通过结构化外部索引优化智能体在长链条推理中的检索效率。
method: 提出“图书馆定理”，通过I/O页面模型对比了索引检索与顺序扫描的理论复杂度。
result: 索引化智能体实现了近乎常数级的检索成本，但模型在处理熟悉内容时易受参数化记忆干扰。
conclusion: 建议将LLM的语义理解能力用于索引构建，而将索引遍历过程交由确定性算法执行。
---
