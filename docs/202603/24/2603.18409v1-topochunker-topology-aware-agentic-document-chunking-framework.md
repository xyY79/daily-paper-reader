...
tldr: 针对检索增强生成（RAG）中传统线性分块导致语义碎片化的问题，本文提出TopoChunker框架。该框架通过结构化中间表示（SIR）保留文档的拓扑层级，并采用双代理架构（Inspector和Refiner）在保证结构完整性的同时优化计算成本。实验表明，该方法在生成准确率和检索召回率上均显著优于现有LLM基线，且大幅降低了Token开销。
