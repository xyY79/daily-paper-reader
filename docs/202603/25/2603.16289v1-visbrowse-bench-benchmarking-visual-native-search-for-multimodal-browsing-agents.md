...
tldr: "针对现有浏览器智能体基准测试在视觉推理能力评估及网页原生视觉信息利用方面的不足，本文提出了VisBrowse-Bench。该基准包含169个跨领域VQA实例，通过文本-图像检索和联合推理验证模型的视觉搜索能力。研究还提出了一种驱动智能体主动收集视觉信息的搜索工作流。实验显示，即使是顶尖模型如Claude-4.6-Opus和o3-deep-research，其准确率也仅在40%-50%之间，凸显了视觉原生搜索的挑战性。"
selection_source: fresh_fetch
motivation: 现有基准测试缺乏对浏览器智能体视觉推理能力的深度评估，且忽视了网页中原生的视觉信息。
method: 构建了包含169个实例的VisBrowse-Bench基准，并提出一种支持主动收集与推理视觉信息的智能体搜索工作流。
result: "实验表明当前最先进的模型表现不佳，Claude-4.6-Opus和o3-deep-research的准确率分别仅为47.6%和41.1%。"
conclusion: VisBrowse-Bench揭示了现有模型在处理视觉原生搜索任务时的巨大局限性，为未来多模态浏览器智能体的发展提供了重要参考。
---

## 摘要
多模态大语言模型（MLLMs）的飞速发展使浏览智能体能够获取并推理现实世界中的多模态信息。然而，现有的基准测试存在两个局限性：对视觉推理能力的评估不足，以及在推理链中忽略了网页的原生视觉信息。为了应对这些挑战，我们引入了一个新的视觉原生搜索基准测试——VisBrowse-Bench。它包含覆盖多个领域的 169 个 VQA 实例，并通过文本-图像检索和联合推理进行多模态证据交叉验证，从而评估模型在搜索过程中的视觉推理能力。这些数据由人类专家通过多阶段流水线构建，并经过了严格的人工验证。此外，我们还提出了一种智能体工作流，能够有效驱动浏览智能体在搜索过程中主动收集并推理视觉信息。我们在该工作流中对开源和闭源模型进行了全面评估。实验结果显示，即使是表现最好的模型 Claude-4.6-Opus 也仅达到了 47.6% 的准确率，而专有的深度研究模型 o3-deep-research 的准确率仅为 41.1%。代码和数据可通过以下链接访问：https://github.com/ZhengboZhang/VisBrowse-Bench。
