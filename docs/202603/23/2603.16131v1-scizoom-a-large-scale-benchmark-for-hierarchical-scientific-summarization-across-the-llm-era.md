...
title_zh: SciZoom：跨越 LLM 时代的大规模分层科学摘要基准
...
## 摘要
AI 研究的爆炸式增长造成了前所未有的信息过载，增加了对传统摘要之外多粒度科学摘要的需求。尽管 LLM 已越来越多地被用于摘要生成，但现有的基准在规模上仍然有限，仅针对单一粒度，且早于 LLM 时代。此外，自 2022 年 11 月 ChatGPT 发布以来，研究人员迅速采用 LLM 来起草手稿，从根本上改变了科学写作，但目前尚无资源用于分析这种写作方式是如何演变的。为了弥补这些空白，我们推出了 SciZoom，这是一个包含来自四个顶级机器学习会议（NeurIPS、ICLR、ICML、EMNLP）的 44,946 篇论文的基准，时间跨度为 2020 年至 2025 年，并明确划分为前 LLM 时代和后 LLM 时代。SciZoom 提供了三个层级的摘要目标（Abstract、Contributions 和 TL;DR），压缩比高达 600:1，既能支持多粒度摘要研究，也能支持科学写作模式的时间挖掘。我们的语言学分析揭示了短语模式（公式化表达增加了多达 10 倍）和修辞风格（避险性语言减少了 23%）的显著变化，这表明 LLM 辅助写作产生了更自信但更同质化的文字。SciZoom 既是一个具有挑战性的基准，也是挖掘生成式 AI 时代科学论述演变的独特资源。我们的代码和数据集已分别在 GitHub (https://github.com/janghana/SciZoom) 和 Hugging Face (https://huggingface.co/datasets/hanjang/SciZoom) 上公开。
