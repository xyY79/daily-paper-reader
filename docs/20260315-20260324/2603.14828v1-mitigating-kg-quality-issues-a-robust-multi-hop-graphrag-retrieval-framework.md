...
tldr: 针对知识图谱（KG）中存在的噪声和不完整性导致的检索漂移与幻觉问题，本文提出 C2RAG 框架。该框架通过将查询分解为原子约束三元组进行精细化过滤，并引入充分性检查机制来决定是否进行结构传播或启动文本恢复。实验证明，C2RAG 在多跳问答任务中显著优于现有基准，有效提升了在不完美 KG 环境下的检索鲁棒性。
selection_source: fresh_fetch
motivation: 现有的 GraphRAG 方法在面对包含噪声和缺失信息的低质量知识图谱时，容易产生检索漂移和幻觉。
method: 提出 C2RAG 框架，通过原子约束锚定过滤候选节点，并利用充分性检查机制结合文本恢复来增强检索的准确性。
result: "在多跳基准测试中，C2RAG 平均在 EM 和 F1 指标上分别超越最新基准 3.4% 和 3.9%。"
conclusion: C2RAG 通过约束校验和动态恢复策略，显著增强了多跳检索在应对知识图谱质量缺陷时的鲁棒性。
---
